import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  // Brand Colors
  static const Color primary = Color(0xFF072C52);
  static Color secondary = Color(0xFF403326);
  static Color secondaryBottomNavbar = Color(0xFFF5EDE0);

  // Gradient Colors
  static const Gradient linearGradient = LinearGradient(
    begin: Alignment(0.0, 0.0),
    end: Alignment(0.707, -0.707),
    colors: [Color(0xfffffa9e), Color(0xFFFAD0C4), Color(0xFFFAD0C4)],
  );

  // Text Colors
  static const Color textPrimary = Color(0xFFFFFFFF);
  static const Color hintText = Color(0xFF9BA4B0);

  static const Color textSecondary = Color(0xFF636F85);
  static const Color textThird = Color(0xFF9BA4B0);
  static const Color textBlue = Color(0xFF2196F3);
  static const Color textBlueE0 = Color(0xFF4DAEE0);
  static const Color textYellow = Color(0xffFFAB4C);
  static const Color textGrey = Color(0xFF94A3B8);
  static const Color textWhite = Color(0xFFFFFFFF);
  static const Color black = Color(0xff000000);
  static const Color blackText = Color(0xff403326);

  static const Color liteGoldenText6C = Color(0xff937C6C);

  static const Color liteGolden = Color(0xff998875);
  static const Color liteGoldenD7 = Color(0xffEAE3D7);
  static const Color golden75 = Color(0xff998875);
  static const Color goldenB7 = Color(0xffD7C8B7);
  static const Color goldenEA = Color(0xffF6F1EA);
  static const Color golden3D = Color(0xffC28F3D);
  static const Color whiteGoldenF8 = Color(0xffFCFBF8);
  static const Color gray = Color(0xff64748B);

  // Border
  static const Color textFieldBorder = Color(0xFFE2E8F0);
  static const Color buttonBorder = Color(0xFFD1D6DB);
  static const Color buttonBorder1 = Color(0xFFEDE6DE);
  static const Color buttonBorder2 = Color(0xFFEAE2D7);

  // Background Colors
  static const Color primaryBackGround = Color(0xffFFF8F0);
  static const Color secondaryBackGround = Color(0xFFDEF7F8);
  static const Color backgroundLight = Color(0xFFF9FAFB);
  static const Color backgroundDark = Color(0xFF121212);

  // Surface Colors
  static const Color surfaceLight = Color(0xFFE0E0E0);
  static const Color surfaceDark = Color(0xFF2C2C2C);

  // Container Colors
  static const Color containerBackground = Color(0xFFD9D9D9);
  static const Color containerBackground2 = Color(0xFFF3F4F6);
  static const Color containerBackground1 = Color(0xFFF9F9FB);
  static const Color containerBackground3 = Color(0xFFEDE6DE);
  static const Color containerBackground4 = Color(0xFFFDEDF6);
  static const Color containerBackground5 = Color(0xFFFFF5E6);

  // Utility Colors
  static const Color success = Color(0xFF4CAF50);
  static const Color dividerColor = Color(0xFFF3F4F6);
  static const Color warning = Color(0xFFFFA726);
  static const Color error = Color(0xFFF44336);
  static const Color redE6 = Color(0xFFFFE6E6);
  static const Color liteRed = Color(0xFFFF7B61);
  static const Color liteRedE6 = Color(0xFFFFE6E6);
  static const Color info = Color(0xFF29B6F6);

  /// textformfield border color

  static const Color textFormFieldBorder = Color(0xFFD1D6DB);

  // Button
  static const Color primaryButton = Color(0xFF6BAFB2);
}
